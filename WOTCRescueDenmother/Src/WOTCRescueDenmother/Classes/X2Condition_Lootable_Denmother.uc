class X2Condition_Lootable_Denmother extends X2Condition_Lootable;

// Same as original, just fail condition on psi loot.
...
event name CallMeetsCondition(XComGameState_BaseObject kTarget)
{
	...
	if (LootObject != none && LootObject.HasAvailableLoot() && LootObject.HasNonPsiLoot())
		return 'AA_Success';
...
{	
	...
	if (bRestrictRange)
	{
		...
		if( TargetLootableState != none )
		{
			...
			else if ( !TargetLootableState.HasAvailableLootForLooter(SourceUnitState.GetReference()) && TargetLootableState.HasNonPsiLoot())
			{
				return 'AA_TargetHasNoLoot';
			}
		}
