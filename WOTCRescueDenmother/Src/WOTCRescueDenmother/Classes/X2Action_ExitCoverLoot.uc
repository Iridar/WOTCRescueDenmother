...
function Init()
{
	...
	History = `XCOMHISTORY;
	PrimaryTarget = History.GetVisualizer(AbilityContext.InputContext.PrimaryTarget.ObjectID);
	LootTarget = Lootable(History.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID));
	LootTileLocation = LootTarget.GetLootLocation();
	TargetLocation = `XWORLD.GetPositionFromTileCoordinates(LootTileLocation);
}
